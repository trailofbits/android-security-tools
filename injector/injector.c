...
long RunLibhook(pid_t pid, const char* library_path) {
  if (DEBUG) {
    printf("Injection started...\n");
  }
  PtraceAttach(pid);
  ...
  long libhook_main = CallDlsym(pid, so_handle, "libhook_main");
  printf("libhook_main loaded at addr %lx\n", libhook_main);
  CallRemoteFunction(pid, libhook_main, NULL, 0);
